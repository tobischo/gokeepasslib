...
func TestNewTimeData(t *testing.T) {
	...
	if time.Since(time.Time(timeData.CreationTime.Time)) > time.Second {
		t.Error("CreationTime not properly initialized: should be time.Now()")
	}
	if time.Since(time.Time(timeData.LastModificationTime.Time)) > time.Second {
		t.Error("LastModificationTime not properly initialized: should be time.Now()")
	}
	if time.Since(time.Time(timeData.LastAccessTime.Time)) > time.Second {
	...
	if time.Since(time.Time(timeData.LocationChanged.Time)) > time.Second {
